...
describe('mtd_cost with quota', () => {
  ...
  test('projection > 90% of entitlement — yellow-orange color and bold present', () => {
    const result = mtd_cost(stdinData, sd({
      quotaEntitlement: 100,
      quotaRemaining:   9,  // used = $0.91/cr → proj well over $1 entitlement
    }), plainOpts);
    ...
    assert.ok(result.includes('\x1b[1m'), `expected bold in: ${JSON.stringify(result)}`);
  });
